...
template <typename T> struct __jule_Ptr;

// Equavelent of Jule's new(T) call.
template <typename T> inline __jule_Ptr<T> __jule_new_ptr(void) noexcept;
...
template <typename T> struct __jule_Ptr {
    ...
    static __jule_Ptr<T> make(const T &instance, __jule_Uint *ref) noexcept {
        ...
        buffer.alloc = static_cast<T *>(__jule_malloc(sizeof(T), false));
    ...
    __jule_Ptr(const __jule_Ptr<T> &src) noexcept { this->__get_copy(src); }

    __jule_Ptr(__jule_Ptr<T> &&src) noexcept {
        this->__get_copy(std::move(src));
    }

    __jule_Ptr(T *src) noexcept { this->alloc = src; }

    ~__jule_Ptr(void) noexcept { this->dealloc(); }
    ...
    inline T &get(void) const noexcept { return *this->alloc; }

    template <typename T2> __jule_Ptr<T2> as(void) const noexcept {
        __jule_Ptr<T2> ptr;
        ptr.ref = this->ref;
#ifndef __JULE_DISABLE__REFERENCE_COUNTING
        if (this->ref)
    ...
    template <typename T2> __jule_Ptr<T2> __as(void) const noexcept {
        __jule_Ptr<T2> ptr;
        ptr.ref = this->ref;
        ptr.alloc = reinterpret_cast<T2 *>(this->alloc);
        return ptr;
    ...
    inline T *operator->(void) const noexcept { return this->alloc; }
    ...
    inline operator T *(void) const noexcept { return this->alloc; }
        ...
        if (this->operator==(nullptr)) {
            if (file != nullptr) {
                ...
                char *message = static_cast<char *>(
                    __jule_malloc((87 + n + 1) * sizeof(char), false));
                if (!message) [[unlikely]] {
                    __jule_panic(
                        (__jule_U8
                ...
                (void)std::strncpy(message, __JULE_ERROR__INVALID_MEMORY, 49);
                (void)std::strncpy(
                    message + 49,
                    "\nruntime: smart pointer is nil\nfile: ", 37);
                (void)std::strncpy(message + 87, file, n);
                message[87 + n] = '\0';

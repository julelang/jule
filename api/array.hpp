...
// Built-in array type.
template <typename Item, __jule_Int N> struct __jule_Array {
public:
    static_assert(N >= 0);
    ...
    constexpr Iterator begin(void) noexcept { return this->buffer; }

    constexpr ConstIterator begin(void) const noexcept { return this->buffer; }

    constexpr Iterator end(void) noexcept { return this->begin() + N; }

    constexpr ConstIterator end(void) const noexcept {
        return this->begin() + N;
    }

    constexpr Iterator hard_end(void) noexcept { return this->end(); }
    ...
    __jule_Slice<Item> slice(const __jule_Int &start,
    ...
    constexpr __jule_Int len(void) const noexcept { return N; }

    constexpr __jule_Bool empty(void) const noexcept { return N == 0; }

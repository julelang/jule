...
// but it actually points to static data for trait's runtime data type if type
// is trait. So, compiler may cast it to actual data type to use it. Therefore,
// the first field of the static data is should be always common function
// pointers.
class __jule_Any {
public:
...
#else
        this->data = __jule_Ptr<__jule_Uintptr>::make(
            reinterpret_cast<__jule_Uintptr *>(alloc));
#endif
    }
    ...
    inline __jule_Any &must_ok_type(const char *file,
    ...
    template <typename T> inline T cast(void) const noexcept {
        return *reinterpret_cast<T *>(this->data.alloc);
    }

    template <typename T> __jule_Ptr<T> cast_ptr(void) const noexcept {

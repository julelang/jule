...
public:
    ...
    str_julet(const char *_Src) noexcept {
        if (!_Src) { return; }
        this->_buffer = std::basic_string<u8_julet>(&_Src[0], &_Src[std::strlen(_Src)]);
    }
    ...
    slice<str_julet> split(const str_julet &_Sub, const i64_julet &_N) const noexcept {
        ...
        if (_N < 0) {
            ...
            if (!_parts.empty()) { _parts.__push(str_julet{_s}); }
        } else {
            uint_julet _n{0};
            while ((_pos = _s.find(_Sub._buffer)) != std::string::npos) {
                _parts.__push(_s.substr(0, _pos));
            ...
            if (!_parts.empty() && _n < _N) { _parts.__push(str_julet{_s}); }
        }
        return _parts;
    }

...
type Analyzer struct {
	...
	allowBuiltin     bool
	package_files    *[]*Analyzer // This field should be not nil.
...
func analyze_file(finf *parser.FileInfo, package_files *[]*Analyzer, used *[]*ast.UseDecl, check bool) *Analyzer {
	p := new_parser()
	p.File = finf.File

	if len(finf.Errors) > 0 {
		p.Errors = append(p.Errors, finf.Errors...)
		return p
	}
...
func AnalyzeFile(finf *parser.FileInfo) *Analyzer {
	return analyze_file(finf, nil, nil, true)
}

func analyze_package(pinf *parser.PackageInfo, used *[]*ast.UseDecl, defs *ast.Defmap) *Analyzer {
	if len(pinf.Files) == 0 {
		return nil
	}

	p := analyze_file(pinf.Files[0], nil, used, false)
	if defs != nil {
		defs.PushDefines(p.Defines)
	}

	for _, f := range pinf.Files[1:] {
		if len(f.Errors) > 0 {
			p.Errors = append(p.Errors, f.Errors...)
			continue
		}
		*p.package_files = append(*p.package_files, analyze_file(f, p.package_files, p.Used, false))
	}
...
func AnalyzePackage(pinf *parser.PackageInfo) *Analyzer {
	return analyze_package(pinf, nil, nil)
}

func new_parser() *Analyzer {
	p := &Analyzer{
		allowBuiltin: true,
		Defines:      &ast.Defmap{},
		Used:         &[]*ast.UseDecl{},
...
func (p *Analyzer) setup_package() {
	p.package_files = &[]*Analyzer{p}
}

// pusherrtok appends new error by token.
func (p *Analyzer) pusherrtok(tok lex.Token, key string, args ...any) {
	p.pusherrmsgtok(tok, build.Errorf(key, args...))
}

// pusherrtok appends new error message by token.
func (p *Analyzer) pusherrmsgtok(tok lex.Token, msg string) {
...
func (p *Analyzer) pusherrs(errs ...build.Log) {
	p.Errors = append(p.Errors, errs...)
}

// PushErr appends new error.
func (p *Analyzer) PushErr(key string, args ...any) {
	p.pusherrmsg(build.Errorf(key, args...))
}

// pusherrmsh appends new flat error message
func (p *Analyzer) pusherrmsg(msg string) {
	p.Errors = append(p.Errors, build.Log{
		Type: build.FLAT_ERR,
		Text: msg,
	})
...
func (p *Analyzer) checkCppUsePath(use *ast.UseDecl) bool {
...
func (p *Analyzer) checkPureUsePath(use *ast.UseDecl) bool {
	info, err := os.Stat(use.Path)
	// Exist?
	if err != nil || !info.IsDir() {
		p.pusherrtok(use.Token, "use_not_found", use.Path)
...
func (p *Analyzer) checkUsePath(use *ast.UseDecl) bool {
	if use.Cpp {
...
func (p *Analyzer) pushSelects(u *ast.UseDecl, selectors []lex.Token) (addNs bool) {
...
func (p *Analyzer) pushUse(use *ast.UseDecl, selectors []lex.Token) {
	if use.FullUse {
...
func (p *Analyzer) compile_cpp_link_use(ast *ast.UseDecl) *ast.UseDecl {
	ast.Cpp = true
	return ast
}
...
func (p *Analyzer) compile_pure_use(ast *ast.UseDecl) (_ *ast.UseDecl, hassErr bool) {
	pinf, err_msg := parser.ParsePackage(ast.Path)
	if err_msg != "" {
		p.pusherrmsg(err_msg)
		return nil, true
	}
	...
	dm := std_builtin_defines[ast.LinkString]
	psub := analyze_package(pinf, p.Used, dm)
...
func (p *Analyzer) compile_use(ast *ast.UseDecl) (*ast.UseDecl, bool) {
	if ast.Cpp {
		return p.compile_cpp_link_use(ast), false
	}
	return p.compile_pure_use(ast)
}

func (p *Analyzer) use_decl(decl *ast.UseDecl, err *bool) {
...
func (p *Analyzer) parseUses(tree *[]ast.Node) (err bool) {
	for i := range *tree {
		node := &(*tree)[i]
...
func (p *Analyzer) parseSrcTreeNode(node ast.Node) {
...
func (p *Analyzer) parseTree(tree []ast.Node) (ok bool) {
	if p.parseUses(&tree) {
		return false
	}
	p.parseSrcTree(tree)
...
func (p *Analyzer) Type(alias TypeAlias) {
...
func (p *Analyzer) parse_enum_items_str(e *Enum) {
...
func (p *Analyzer) parse_enum_items_integer(e *Enum) {
...
func (p *Analyzer) pushField(s *Struct, f **Var, i int) {
...
func (p *Analyzer) parseFields(s *Struct) {
...
func (p *Analyzer) Struct(model Struct) {
...
func (p *Analyzer) LinkFn(link ast.CppLinkFn) {
...
func (p *Analyzer) Link_alias(link ast.CppLinkAlias) {
	if lex.IsIgnoreId(link.Link.Id) {
		p.pusherrtok(link.Token, "ignore_id")
		return
	}
...
func (p *Analyzer) Link_struct(link ast.CppLinkStruct) {
	if lex.IsIgnoreId(link.Link.Id) {
		p.pusherrtok(link.Token, "ignore_id")
		return
	}
...
func (p *Analyzer) LinkVar(link ast.CppLinkVar) {
	if lex.IsIgnoreId(link.Link.Id) {
		p.pusherrtok(link.Token, "ignore_id")
		return
	}
...
func (p *Analyzer) Trait(model ast.Trait) {
	if lex.IsIgnoreId(model.Id) {
		p.pusherrtok(model.Token, "ignore_id")
		return
	} else if def, _, _ := p.defined_by_id(model.Id); def != nil {
...
func (p *Analyzer) implTrait(model *ast.Impl) {
...
func (p *Analyzer) check_impl_generics(s *ast.Struct, types []*GenericType) {
	if len(s.Generics) > 0 {
		for _, t := range types {
...
func (p *Analyzer) implStruct(model *ast.Impl) {
	side := p.Defines.Side
	p.Defines.Side = nil
...
func (p *Analyzer) Impl(impl *ast.Impl) {
...
func (p *Analyzer) pushNs(identifiers []string) *ast.Namespace {
...
func (p *Analyzer) parseFnNonGenericType(generics []*GenericType, dt *Type) {
	f := dt.Tag.(*Fn)
...
func (p *Analyzer) parseMultiNonGenericType(generics []*GenericType, dt *Type) {
	types := dt.Tag.([]Type)
	for i := range types {
		mt := &types[i]
...
func (p *Analyzer) parseMapNonGenericType(generics []*GenericType, dt *Type) {
	p.parseMultiNonGenericType(generics, dt)
}

func (p *Analyzer) parseCommonNonGenericType(generics []*GenericType, dt *Type) {
	if dt.Id == types.ID {
		id, prefix := dt.KindId()
...
func (p *Analyzer) parseNonGenericType(generics []*GenericType, dt *Type) {
	switch {
	case dt.MultiTyped:
		p.parseMultiNonGenericType(generics, dt)
	case types.IsFn(*dt):
...
func (p *Analyzer) parseTypesNonGenerics(f *Fn) {
	for i := range f.Params {
...
func (p *Analyzer) check_ret_variables(f *Fn) {
...
func (p *Analyzer) function(ast Fn) {
...
func (p *Analyzer) global(vast Var) {
...
func (p *Analyzer) variable(model Var) *Var {
	if lex.IsIgnoreId(model.Id) {
		p.pusherrtok(model.Token, "ignore_id")
	}
	v := new(Var)
...
func (p *Analyzer) varsFromParams(f *Fn) []*Var {
	length := len(f.Params)
...
func (p *Analyzer) linked_alias_by_id(id string) *ast.TypeAlias {
	for _, fp := range *p.package_files {
...
func (p *Analyzer) linked_struct_by_id(id string) *Struct {
	for _, fp := range *p.package_files {
...
func (p *Analyzer) linkedVarById(id string) *Var {
	for _, fp := range *p.package_files {
...
func (p *Analyzer) linkedFnById(id string) *ast.Fn {
	for _, fp := range *p.package_files {
...
//	's' -> struct
//	't' -> type alias
func (p *Analyzer) linkById(id string) (any, byte) {
	f := p.linkedFnById(id)
	if f != nil {
		return f, 'f'
	}
...
func (p *Analyzer) fn_by_id(id string) (*Fn, *ast.Defmap, bool) {
	if p.allowBuiltin {
...
func (p *Analyzer) global_by_id(id string) (*Var, *ast.Defmap, bool) {
	for _, fp := range *p.package_files {
...
func (p *Analyzer) NsById(id string) *ast.Namespace { return p.Defines.NsById(id) }

// Reports identifier is shadowed or not.
func (p *Analyzer) is_shadowed(id string) bool {
	def, _, _ := p.block_define_by_id(id)
	return def != nil
}

func (p *Analyzer) is_accessible_define(fp *Analyzer, dm *ast.Defmap) bool {
...
func (p *Analyzer) type_by_id(id string) (*TypeAlias, *ast.Defmap, bool) {
...
func (p *Analyzer) enum_by_id(id string) (*Enum, *ast.Defmap, bool) {
	if p.allowBuiltin {
...
func (p *Analyzer) struct_by_id(id string) (*Struct, *ast.Defmap, bool) {
	if p.allowBuiltin {
...
func (p *Analyzer) trait_by_id(id string) (*ast.Trait, *ast.Defmap, bool) {
	if p.allowBuiltin {
...
func (p *Analyzer) block_type_by_id(id string) (_ *TypeAlias, can_shadow bool) {
...
func (p *Analyzer) block_var_by_id(id string) (_ *Var, can_shadow bool) {
...
func (p *Analyzer) defined_by_id(id string) (def any, tok lex.Token, canshadow bool) {
	var a *TypeAlias
...
func (p *Analyzer) block_define_by_id(id string) (def any, tok lex.Token, canshadow bool) {
	bv, canshadow := p.block_var_by_id(id)
	if bv != nil {
		return bv, bv.Token, canshadow
	}
...
func (p *Analyzer) precheck_package() {
...
func (p *Analyzer) parse_package_defines() {
...
func (p *Analyzer) parse_defines() {
	p.check_structs()
	p.check_fns()
}

func (p *Analyzer) check_package() {
	p.precheck_package()
	if !p.JustDefines {
...
func (p *Analyzer) parse_struct(s *Struct) {
	p.parseFields(s)
}

func (p *Analyzer) parse_package_structs() {
	for _, pf := range *p.package_files {
		pf.parse_structs()
		if p != pf {
			pf.wg.Wait()
...
func (p *Analyzer) parse_package_linked_structs() {
	for _, pf := range *p.package_files {
...
func (p *Analyzer) check_package_linked_aliases() {
	for _, pf := range *p.package_files {
...
func (p *Analyzer) check_package_linked_vars() {
	for _, pf := range *p.package_files {
...
func (p *Analyzer) check_package_linked_fns() {
	for _, pf := range *p.package_files {
		pf.check_linked_fns()
		if p != pf {
			pf.wg.Wait()
...
func (p *Analyzer) check_linked_fns() {
...
func (p *Analyzer) parse_package_aliases() {
	for _, pf := range *p.package_files {
		pf.parse_aliases()
		if p != pf {
			pf.wg.Wait()
...
func (p *Analyzer) parse_package_waiting_fns() {
	for _, pf := range *p.package_files {
		pf.ParseWaitingFns()
		if p != pf {
			pf.wg.Wait()
...
func (p *Analyzer) ParseWaitingFns() {
...
func (p *Analyzer) parse_package_waiting_globals() {
	for _, pf := range *p.package_files {
...
func (p *Analyzer) ParseWaitingGlobals() {
	for _, g := range p.Defines.Globals {
		*g = *p.variable(*g)
	}
}

func (p *Analyzer) parse_package_waiting_impls() {
	for _, pf := range *p.package_files {
...
func (p *Analyzer) ParseWaitingImpls() {
...
func (p *Analyzer) checkParamDefaultExprWithDefault(param *Param) {
	if types.IsFn(param.DataType) {
		p.pusherrtok(param.Token, "invalid_type_for_default_arg", param.DataType.Kind)
	}
}

func (p *Analyzer) checkParamDefaultExpr(f *Fn, param *Param) {
...
func (p *Analyzer) param(f *Fn, param *Param) (err bool) {
	p.checkParamDefaultExpr(f, param)
	return
}

func (p *Analyzer) check_param_dup(params []ast.Param) (err bool) {
...
func (p *Analyzer) params(f *Fn) (err bool) {
	hasDefaultArg := false
	for i := range f.Params {
		param := &f.Params[i]
...
func (p *Analyzer) block_variables_of_fn(f *Fn) []*Var {
...
func (p *Analyzer) parse_pure_fn(f *Fn) (err bool) {
	hasError := p.eval.has_error
	owner := f.Owner.(*Analyzer)
	err = owner.params(f)
	if err {
		return
	}
...
func (p *Analyzer) parse_fn(f *Fn) (err bool) {
	if len(f.Generics) > 0 {
		return false
	}
...
func (p *Analyzer) parse_struct_fn(s *Struct, f *Fn) (err bool) {
	if len(f.Generics) > 0 {
		return
	}
	if len(s.Generics) == 0 {
...
func (p *Analyzer) checkStruct(xs *Struct) (err bool) {
...
func (p *Analyzer) check_fn_special_cases(f *Fn) {
	switch f.Id {
...
func (p *Analyzer) call_fn(f *Fn, data call_data, m *expr_model) value {
	v := p.parse_fn_call_toks(f, data.generics, data.args, m)
	v.lvalue = types.IsLvalue(v.data.DataType)
	return v
}

func (p *Analyzer) callStructConstructor(s *Struct, argsToks []lex.Token, m *expr_model) (v value) {
	f := s.Constructor
...
func (p *Analyzer) parse_field(s *Struct, f **Var, i int) {
	*f = p.variable(**f)
	v := *f
...
func (p *Analyzer) structConstructorInstance(as *Struct) *Struct {
	s := new(Struct)
	*s = *as
	s.Origin = as
...
func (p *Analyzer) check_anon_fn(f *Fn) {
	_ = p.check_param_dup(f.Params)
	p.check_ret_variables(f)
	p.reload_fn_types(f)
...
func (p *Analyzer) get_args(toks []lex.Token, targeting bool) *ast.Args {
	toks, _ = p.get_range(lex.KND_LPAREN, lex.KND_RPARENT, toks)
	if toks == nil {
		toks = make([]lex.Token, 0)
	}
	args, errors := parser.BuildArgs(toks, targeting)
	if len(errors) > 0 {
		p.pusherrs(errors...)
		args = nil
	}
...
func (p *Analyzer) get_generics(toks []lex.Token) (_ []Type, err bool) {
	if len(toks) == 0 {
		return nil, false
	}
	// Remove braces
	...
	for i, part := range parts {
		...
		j := 0
		generic, errors := parser.BuildType(part, &j)
		if j+1 < len(part) {
...
func (p *Analyzer) checkGenericsQuantity(required, given int, errTok lex.Token) bool {
...
func (p *Analyzer) pushGeneric(generic *GenericType, source Type, errtok lex.Token) {
...
func (p *Analyzer) pushGenerics(generics []*GenericType, sources []Type, errtok lex.Token) {
	for i, generic := range generics {
		p.pushGeneric(generic, sources[i], errtok)
	}
}

func (p *Analyzer) fn_parse_type(t *Type) bool {
...
func (p *Analyzer) reload_fn_types(f *Fn) {
	for i := range f.Params {
...
func (p *Analyzer) parseGenericFn(f *Fn, args *ast.Args, errtok lex.Token) {
...
func (p *Analyzer) parseGenerics(f *Fn, args *ast.Args, errTok lex.Token) bool {
...
func (p *Analyzer) parse_fn_call(f *Fn, args *ast.Args, m *expr_model, errTok lex.Token) (v value) {
...
func (p *Analyzer) parse_fn_call_toks(f *Fn, genericsToks, argsToks []lex.Token, m *expr_model) (v value) {
	var generics []Type
	var args *ast.Args
	var err bool
...
func (p *Analyzer) parseStructArgs(f *Fn, args *ast.Args, errTok lex.Token) {
	sap := structArgParser{
		p:      p,
		f:      f,
		args:   args,
...
func (p *Analyzer) parsePureArgs(f *Fn, args *ast.Args, m *expr_model, errTok lex.Token) {
	pap := pureArgParser{
		p:      p,
		f:      f,
		args:   args,
...
func (p *Analyzer) parseArgs(f *Fn, args *ast.Args, m *expr_model, errTok lex.Token) {
	if args.Targeted {
...
func (p *Analyzer) pushGenericByFunc(f *Fn, pair *paramMapPair, args *ast.Args, gt Type) bool {
	tf := gt.Tag.(*Fn)
...
func (p *Analyzer) pushGenericByMap(f *Fn, pair *paramMapPair, args *ast.Args, gt Type) bool {
...
func (p *Analyzer) pushGenericByMultiTyped(f *Fn, pair *paramMapPair, args *ast.Args, gt Type) bool {
	_types := gt.Tag.([]Type)
...
func (p *Analyzer) pushGenericByCommonArg(f *Fn, pair *paramMapPair, args *ast.Args, t Type) bool {
...
func (p *Analyzer) pushGenericByType(f *Fn, g *GenericType, pos int, args *ast.Args, gt Type) {
	id, _ := gt.KindId()
	gt.Kind = id
	f.Owner.(*Analyzer).pushGeneric(g, gt, f.Token)

	// Add by order.
	args.Generics[pos] = gt
}

func (p *Analyzer) pushGenericByComponent(f *Fn, pair *paramMapPair, args *ast.Args, argType Type) bool {
...
func (p *Analyzer) pushGenericByArg(f *Fn, pair *paramMapPair, args *ast.Args, argType Type) bool {
...
func (p *Analyzer) check_arg(f *Fn, pair *paramMapPair, args *ast.Args, variadiced *bool, v value) {
...
func (p *Analyzer) parse_arg(f *Fn, pair *paramMapPair, args *ast.Args, variadiced *bool) {
	var v value
...
func (p *Analyzer) check_assign_type(real Type, val value, errTok lex.Token) {
	assign_checker{
		p:      p,
		t:      real,
		v:      val,
...
func (p *Analyzer) checkArgType(param *Param, val value, errTok lex.Token) {
...
func (p *Analyzer) get_range(open, close string, toks []lex.Token) (_ []lex.Token, ok bool) {
	i := 0
	toks = ast.Range(&i, open, close, toks)
	return toks, toks != nil
}

func (p *Analyzer) checkSolidFuncSpecialCases(f *Fn) {
	if len(f.Params) > 0 {
...
func (p *Analyzer) checkNewBlockCustom(b *ast.Block, oldBlockVars []*Var) {
...
func (p *Analyzer) checkNewBlock(b *ast.Block) {
	p.checkNewBlockCustom(b, p.block_vars)
}

func (p *Analyzer) fall_st(f *ast.Fall, i *int) {
	switch {
...
// It's safe.
func (p *Analyzer) common_st(s *ast.St, i *int, recover bool) bool {
...
func (p *Analyzer) check_st(i *int) {
...
// Check p.nodeBlock
func (p *Analyzer) checkNodeBlock() {
...
func (p *Analyzer) recover_fn_expr_st(s *ast.ExprSt, i *int) {
...
func (p *Analyzer) expr_st(s *ast.ExprSt, i *int, recover bool) (is_recover bool) {
...
func (p *Analyzer) count_match_type(m *ast.Match, t Type) int {
	n := 0
loop:
...
func (p *Analyzer) parseCase(m *ast.Match, c *ast.Case) {
	for i := range c.Exprs {
		expr := &c.Exprs[i]
		switch expr.Op.(type) {
		case Type:
...
func (p *Analyzer) cases(m *ast.Match) {
	for i := range m.Cases {
		p.parseCase(m, &m.Cases[i])
	}
}

func (p *Analyzer) matchcase(m *ast.Match) {
	if !m.Expr.IsEmpty() {
...
func (p *Analyzer) checkSameScopeGoto(gt *ast.Goto, label *ast.Label) {
...
func (p *Analyzer) checkLabelParents(gt *ast.Goto, label *ast.Label) bool {
	block := label.Block
parent_scopes:
...
func (p *Analyzer) checkGotoScope(gt *ast.Goto, label *ast.Label) {
...
func (p *Analyzer) checkDiffScopeGoto(gt *ast.Goto, label *ast.Label) {
	switch {
...
func (p *Analyzer) checkGoto(gt *ast.Goto, label *ast.Label) {
	switch {
...
func (p *Analyzer) checkLabelNGoto() {
	p.checkGotos()
	p.checkLabels()
}
...
func (p *Analyzer) checkRets(f *Fn) {
...
func (p *Analyzer) concurrent_call(cc *ast.ConcurrentCall) {
	m := new(expr_model)
	m.nodes = make([]expr_build_node, 1)
	_, cc.Expr.Model = p.eval_expr(cc.Expr, nil)
}

func (p *Analyzer) check_assign(left value, errtok lex.Token) bool {
	state := true
	if !left.lvalue {
...
func (p *Analyzer) single_assign(assign *ast.Assign, l, r []value) {
	left := l[0]
	switch {
...
func (p *Analyzer) assign_exprs(ast *ast.Assign) (l []value, r []value) {
		...
		if i < len(l) {
			...
			if !left.Var.New &&
				...
				if !canshadow && def != nil {
					...
					// Parse as assignment to existing variable.
					left.Var.New = false
					set_existing()
				} else {
...
func (p *Analyzer) funcMultiAssign(vsAST *ast.Assign, l, r []value) {
...
func (p *Analyzer) check_valid_init_expr(left_mutable bool, right value, errtok lex.Token) {
...
func (p *Analyzer) multi_assign(assign *ast.Assign, l []value, r []value) {
...
func (p *Analyzer) unsafe_allowed() bool {
	return (p.rootBlock != nil && p.rootBlock.IsUnsafe) ||
		(p.nodeBlock != nil && p.nodeBlock.IsUnsafe)
}

func (p *Analyzer) postfix(assign *ast.Assign, l, r []value) {
	if len(r) > 0 {
...
func (p *Analyzer) assign(assign *ast.Assign) {
	ln := len(assign.Left)
	rn := len(assign.Right)
...
func (p *Analyzer) whileProfile(iter *ast.Iter) {
...
func (p *Analyzer) foreachProfile(iter *ast.Iter) {
...
func (p *Analyzer) iter(iter *ast.Iter) {
	oldCase := p.currentCase
...
func (p *Analyzer) conditional_node(node *ast.If) {
...
func (p *Analyzer) conditional(model *ast.Conditional) {
...
func (p *Analyzer) breakWithLabel(brk *ast.Break) {
...
func (p *Analyzer) continueWithLabel(cont *ast.Continue) {
...
func (p *Analyzer) break_st(ast *ast.Break) {
	switch {
...
func (p *Analyzer) continue_st(ast *ast.Continue) {
	switch {
	case p.currentIter == nil:
...
func (p *Analyzer) checkValidityForAutoType(expr_t Type, errtok lex.Token) {
	if p.eval.has_error {
		return
	}
	switch expr_t.Id {
...
func (p *Analyzer) typeSourceOfMultiTyped(dt Type, err bool) (Type, bool) {
	types := dt.Tag.([]Type)
	ok := false
...
func (p *Analyzer) typeSourceIsAlias(dt Type, alias *TypeAlias, err bool) (Type, bool) {
...
func (p *Analyzer) typeSourceIsEnum(e *Enum, tag any) (dt Type, _ bool) {
	dt.Id = types.ENUM
	dt.Kind = e.Id
	dt.Tag = e
	dt.Token = e.Token
...
func (p *Analyzer) typeSourceIsFn(dt Type, err bool) (Type, bool) {
	f := dt.Tag.(*Fn)
	if len(f.Generics) > 0 {
...
func (p *Analyzer) typeSourceIsMap(dt Type, err bool) (Type, bool) {
	types := dt.Tag.([]Type)
	key := &types[0]
...
func (p *Analyzer) typeSourceIsStruct(s *Struct, st Type) (dt Type, _ bool) {
	generics := s.GetGenerics()
	if len(generics) > 0 {
...
func (p *Analyzer) typeSourceIsTrait(trait_def *ast.Trait, tag any, errTok lex.Token) (dt Type, _ bool) {
	if tag != nil {
...
func (p *Analyzer) tokenizeDataType(id string) []lex.Token {
...
func (p *Analyzer) typeSourceIsArrayType(arr_t *Type) (ok bool) {
	ok = true
	arr_t.Original = nil
	arr_t.Pure = true
...
func (p *Analyzer) typeSourceIsSliceType(slc_t *Type) (ok bool) {
...
func (p *Analyzer) check_type_validity(expr_t Type, errtok lex.Token) {
...
func (p *Analyzer) get_define(id string, cpp_linked bool) any {
	var def any = nil
	if cpp_linked {
		def, _ = p.linkById(id)
...
func (p *Analyzer) typeSource(dt Type, err bool) (ret Type, ok bool) {
	if dt.Kind == "" {
		return dt, true
	}
	original := dt.Original
...
func (p *Analyzer) realType(dt Type, err bool) (ret Type, _ bool) {
	original := dt.Original
	defer func() {
		ret.CppLinked = (original != nil && original.(Type).CppLinked) || dt.CppLinked
		ret.Original = original
...
func (p *Analyzer) checkMultiType(real, check Type, ignoreAny bool, errTok lex.Token) {
...
func (p *Analyzer) check_type(real, check Type, ignoreAny, allow_assign bool, errTok lex.Token) {
...
func (p *Analyzer) eval_expr(expr Expr, prefix *ast.Type) (value, ast.ExprModel) {
	p.eval.has_error = false
	p.eval.type_prefix = prefix
	return p.eval.eval_expr(expr)
}

func (p *Analyzer) evalToks(toks []lex.Token, prefix *ast.Type) (value, ast.ExprModel) {
	p.eval.has_error = false
	p.eval.type_prefix = prefix
